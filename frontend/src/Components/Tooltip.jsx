...
const Tooltip = ({ className, infoText, children, position = "middle" }) => {
    ...
    return (
        <div className={ `flex flex-col gap-[20px] justify-center relative ${position === "right" ? "items-end" : "items-center"}` }>
            ...
            <div className={ createClassName(`h-fit z-20 top-[-50px] text-wrap absolute w-[309px] rounded-8 px-4 py-8 text-body-s normal-case font-normal text-neutral-0 bg-neutral-900 text-center transition-all ease-in ${showTooltip ? "opacity-100" : "opacity-0"} ${position === "right" ? "" : ""}`, className) }>
                { infoText }
