...
const MovieCard = ({ className, seeAlso, ...props }) => {
    ...
    return (
        <Card className={ createClassName("h-[395px] cursor-pointer", className) } onClick={ () => { navigate('/movie-details', { state: { movie } }) } }>
            <div className="flex items-center justify-center p-12 relative h-[77%]">
                <img className="w-[98%] rounded-16 h-full" src={ cover } alt="" />
