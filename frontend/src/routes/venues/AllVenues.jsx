...
const AllVenues = () => {
    ...
    const getCityName = (id) => cityList?.find(c => c.cityId.toString() === id)?.name;
            ...
            <div className="grid grid-cols-5 gap-12 pb-8">
                ...
                <LabeledDropdown value={ getCityName(filterParams.city) } label={ cityLabel }>
                    ...
                    { cityList.map((city, index) => (
                        <DropdownItem key={ index } onClick={ () => handleFilter('city', city.cityId) } className={ `flex hover:bg-neutral-100 rounded-8 px-12 py-8 cursor-pointer ${city.cityId === parseInt(filterParams.city) ? "font-semibold" : "font-normal"}` }>
                            { city.name }
                        </DropdownItem>
                    )) }
                </LabeledDropdown>
